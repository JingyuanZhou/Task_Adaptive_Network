...
class OperationLayer(nn.Module):
    ...
    def forward(self, x, weights,task_logit):
        ...
        if self.need_ta:
            return self.task_attn(output,task_logit)
        return output
...
class GCADecoder(nn.Module):
    ...
    def forward(self, y1, task_logit):
        ...
        weights=self.oa_1(task_logit)
        y2=self.layer_1(y1,weights,task_logit)
        weights=self.oa_2(task_logit)
        y3=self.layer_2(y2,weights,task_logit)
        weights=self.oa_3(task_logit)
        y4=self.layer_3(y3,weights,task_logit)
        weights=self.oa_4(task_logit)
        y5=self.layer_4(y4,weights,task_logit)

        gates = self.gate(torch.cat((y1, y2, y3 ,y4 ,y5), dim=1))
        gated_y = y1 * gates[:, [0], :, :] + y2 * gates[:, [1], :, :] + y3 * gates[:, [2], :, :]+ y4 * gates[:, [3], :, :] + y5 * gates[:, [4], :, :]
        weights=self.oa_5(task_logit)
        y=self.layer_5(gated_y,weights,task_logit)
        weights=self.oa_6(task_logit)
        y=self.layer_6(y,weights,task_logit)
        weights=self.oa_7(task_logit)
        y=self.layer_7(y,weights,task_logit)
...
class GCANet(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.decoder(out, task_logit1)
        return x + out, task_logit1, task_prob1
        
if __name__ == '__main__':
    model = GCANet()
    model = model.cuda()
    x = torch.rand((4,3,100,100)).cuda()
    y, task_logit1, task_prob1 = model(x)