...
def populate_train_list(orig_images_path, degen_images_path):
    ...
    sub_datasets=['rain/','hazy/','rain_drop_test_a/','rain_drop_test_b/']
    ...
    for sub in sub_datasets:
        if sub == 'rain/':
            ...
            tmp_ls = get_hazy(orig_images_path, degen_images_path, sub,3)
        
        elif sub == 'rain_drop/' or sub == 'rain_drop_test_a/' or sub == 'rain_drop_test_b/':
            tmp_ls = get_rain_drop(orig_images_path, degen_images_path, sub, 6)#12
        '''
        elif sub == 'low_light/':
...
class merge_dataset(data.Dataset):
    def __init__(self,data_list, train=True):
        ...
        self.outsize = (200,200) if self.train else (200,200)
    ...
    def __getitem__(self, index):
        ...
        data = self.transform(image=data_orig, image2=data_degen)
        data_orig, data_degen = data['image'], data['image2']
